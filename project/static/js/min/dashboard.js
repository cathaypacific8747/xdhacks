...
b?.volumeInfo?.publisher;this.publishedDate=b?.volumeInfo?.publishedDate;this.pageCount=b?.volumeInfo?.pageCount;this.height=b?.volumeInfo?.dimensions?.height;this.width=b?.volumeInfo?.dimensions?.width;this.thickness=b?.volumeInfo?.dimensions?.thickness;this.imagelinks=b?.volumeInfo?.imageLinks;this.thumbSmall=this.imagelinks?.smallThumbnail?.replace("http","https");this.thumbLarge=this.imagelinks?.extraLarge?this.imagelinks.extraLarge:this.imagelinks?.large?this.imagelinks.large:this.imagelinks?.medium?
...
"None";break;case 1:this.strings.notes="Some";break;case 2:this.strings.notes="Comprehensive"}this.strings.price=`HKD ${this.price}`;this.strings.remarks=this.remarks||"---"}};class u extends Error{constructor(b){super(b)}}class x extends Error{constructor(b){super(b);this.message=`${b.status}: ${b.statusText}`}}class v extends Error{constructor(b){super(b);this.message=`${b.code}: ${b.message}`}}class w extends Error{constructor(){super();this.message="No results found."}}window.NetworkError=x;window.APIError=
v;window.NoGoogleBooksResultsError=w;window.ControlledError=u;window.toast=function(b){const k=b.description||"An unknown error occurred",q=`${b.headerPrefix} `||"";let p,t;switch(b.code||3){case 3:p=`${q}Error`;t="toastError";break;case 2:p=`${q}Warning`;t="toastWarning";break;case 1:p=`${q}Success`;t="toastSuccess";break;case 0:p=`${q}Information`,t="toastInformation"}M.toast({unsafeHTML:`<div>
