...
"None";break;case 1:this.strings.notes="Some";break;case 2:this.strings.notes="Comprehensive"}this.strings.price=`HKD ${this.price}`;this.strings.remarks=this.remarks||"---"}};class g extends Error{constructor(a){super(a)}}class h extends Error{constructor(a){super(a);this.message=`${a.status}: ${a.statusText}`}}class k extends Error{constructor(a){super(a);this.message=`${a.code}: ${a.message}`}}class d extends Error{constructor(){super();this.message="No results found."}}window.NetworkError=h;window.APIError=
k;window.NoGoogleBooksResultsError=d;window.ControlledError=g;window.toast=function(a){const b=a.description||"An unknown error occurred",e=`${a.headerPrefix} `||"";let f,c;switch(a.code||3){case 3:f=`${e}Error`;c="toastError";break;case 2:f=`${e}Warning`;c="toastWarning";break;case 1:f=`${e}Success`;c="toastSuccess";break;case 0:f=`${e}Information`,c="toastInformation"}M.toast({unsafeHTML:`<div>
