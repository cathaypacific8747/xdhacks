...
$(document).ready(function(){$(".dropdown-trigger").dropdown({alignment:"right",constrainWidth:!1,coverTrigger:!1});$(".tooltipped").tooltip();M.updateTextFields();dayjs.extend(window.dayjs_plugin_utc);dayjs.extend(window.dayjs_plugin_relativeTime);window.Book=class{constructor(c){this.googleId=null==c?void 0:c.id;let g;this.title=null==c?void 0:null==(g=c.volumeInfo)?void 0:g.title;let f,k,h;this.isbn=null==c?void 0:null==(f=c.volumeInfo)?void 0:null==(k=f.industryIdentifiers)?void 0:null==(h=k.find(I=>
"ISBN_13"==I.type))?void 0:h.identifier;let l;this.authors=null==c?void 0:null==(l=c.volumeInfo)?void 0:l.authors;let m;this.language=null==c?void 0:null==(m=c.volumeInfo)?void 0:m.language;let n;this.publisher=null==c?void 0:null==(n=c.volumeInfo)?void 0:n.publisher;let p;this.publishedDate=null==c?void 0:null==(p=c.volumeInfo)?void 0:p.publishedDate;let q;this.pageCount=null==c?void 0:null==(q=c.volumeInfo)?void 0:q.pageCount;let r,t;this.height=null==c?void 0:null==(r=c.volumeInfo)?void 0:null==
...
h="toastWarning";break;case 1:k=`${f}Success`;h="toastSuccess";break;case 0:k=`${f}Information`,h="toastInformation"}M.toast({unsafeHTML:`<div>
            ...
            </div>`,classes:`toastGeneral ${h} roundBox`})};window.toastError=function(c){c instanceof b?toast({description:c.message,headerPrefix:"Network",code:3}):c instanceof d&&toast({description:c.message,headerPrefix:"Server",code:3});console.error(c)};window.removeShimmer=function(c){$(c).removeClass("shimmerBG")};window.removeMinPicHeight=function(c){$(c).removeClass("minPicHeight")};448>$(window).width()&&toast({description:'Elements may appear glitched on mobile. Please use a computer, or try rotating your device into the horizontal orientation. View <a href="/help#master">help</a> for more details.',
headerPrefix:"Incompatibility",code:2})});
$(document).ready(function(){$("#imagemodal").modal({onOpenEnd:()=>{$("#carousel").carousel({indicators:!0})}});var a=$("meta[name=userid]").attr("content");$('[data-element="help"]').removeClass("hide").html("Loading listings...");$('[data-element="progress"]').removeClass("hide");fetch(`/api/v1/listing/detail?userid=${a}`,{method:"GET",mode:"cors",headers:csrfprotect({"Content-Type":"application/json"})}).then(b=>{if(!b.ok)throw new NetworkError;return b.json()}).then(b=>{if("success"!=b.status)throw new APIError(b);
if(0==b.data.length)throw $('[data-element="help"]').html("This user does not have listings."),new ControlledError;return b.data}).then(b=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){return Promise.all(b.map(d=>{({bookid:d}=d);return fetch(`https://www.googleapis.com/books/v1/volumes/${d}`,{method:"GET",mode:"cors",headers:{"Content-Type":"application/json"}})})).then(d=>Promise.all(d.map(e=>{if(!e.ok)throw new NetworkError(e);return e.json()}))).then(d=>Promise.all(d.map(e=>{if("error"in
e)throw new NoGoogleBooksResultsError;return e}))).then(d=>{let e=$('[data-element="listings_results"]');for(let g=0;g<d.length;g++){var c=new Book(d[g]);let f=new Listing(b[g]);c=$(`<div class="row mx-0 mb-8 p-8 roundBox book" data-listingid="${f.listingid}">
                    <div class="col s2 mx-0 p-0 minPicHeight shimmerBG">
                        <img class="google-book-image roundBox" src="${c.strings.thumbSmall}" onload="removeShimmer(this.parentElement);removeMinPicHeight(this.parentElement)" data-field="thumb">
                    </div>
                    <div class="col s10">
                        <div class="row mt-0 mb-2">
                            <div class="col font-size-24 text-bold">${c.strings.title}</div>
                        </div>
                        <div class="row mt-0 mb-2">
                            <div class="col s6">
                                <div class="row my-0">
                                    <div class="col font-size-14 text-muted">Author${c.strings.plurality}</div>
                                </div>
                                <div class="row my-0">
                                    <div class="col font-size-16">${c.strings.authors}</div>
                                </div>
                                <div class="row my-0">
                                    <div class="col font-size-14 text-muted">Publisher</div>
                                </div>
                                <div class="row my-0">
                                    <div class="col font-size-16">${c.strings.publisher}</div>
                                </div>
                                <div class="row my-0">
                                    <div class="col font-size-14 text-muted">ISBN</div>
                                </div>
                                <div class="row my-0">
                                    <div class="col font-size-16">${c.strings.isbn}</div>
                                </div>
                            </div>
                                    ...
                                    <div class="col">
                                        ...
                                        <div class="row my-0">
                                            <div class="col s12 word-wrap font-size-16 right-align">
                                                <span class="${f.strings.conditionClass} tooltipped" data-position="right" data-tooltip="${f.strings.conditionDescription}">
                                                    ${f.strings.condition}
                                                </span>
                                            </div>
                                        </div>
                ...
                </div>`);e.append(c)}$('[data-element="help"]').addClass("hide").empty();$('[data-button="view_image"]').click(g=>{const f=$("#carousel").empty(),k=$(g.target).closest("[data-listingid]").attr("data-listingid");b.find(h=>h.listingid==k).images.forEach(h=>{f.append(`<a class="carousel-item justify-content-center"><img src="${h}"></a>`)});$("#imagemodal").modal("open")})}).catch(d=>{throw d;})})).catch(b=>{b instanceof NoGoogleBooksResultsError?$('[data-element="help"]').html("An error occurred in Google's servers. Please try again later."):
