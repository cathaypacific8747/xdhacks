...
"None";break;case 1:this.strings.notes="Some";break;case 2:this.strings.notes="Comprehensive"}this.strings.price=`HKD ${this.price}`;this.strings.remarks=this.remarks||"---"}};class c extends Error{constructor(a){super(a)}}class e extends Error{constructor(a){super(a);this.message=`${a.status}: ${a.statusText}`}}class f extends Error{constructor(a){super(a);this.message=`${a.code}: ${a.message}`}}class g extends Error{constructor(){super();this.message="No results found."}}window.NetworkError=e;window.APIError=
f;window.NoGoogleBooksResultsError=g;window.ControlledError=c;window.toast=function(a){const b=a.description||"An unknown error occurred",d=`${a.headerPrefix} `||"";let h,k;switch(a.code||3){case 3:h=`${d}Error`;k="toastError";break;case 2:h=`${d}Warning`;k="toastWarning";break;case 1:h=`${d}Success`;k="toastSuccess";break;case 0:h=`${d}Information`,k="toastInformation"}M.toast({unsafeHTML:`<div>
