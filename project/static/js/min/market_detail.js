...
function(a="An unknown error occured",d="",e=3){d=d?`${d} `:"";let f,g;switch(e){case 3:f=`${d}Error`;g="toastError";break;case 2:f=`${d}Warning`;g="toastWarning";break;case 1:f=`${d}Success`;g="toastSuccess";break;case 0:f=`${d}Information`,g="toastInformation"}M.toast({unsafeHTML:`<div>
...
{method:"GET",mode:"cors",headers:{"Content-Type":"application/json"}}).then(b=>{if(!b.ok)throw new NetworkError(b);return b.json()}).then(b=>{if("error"in b)throw new NoGoogleBooksResultsError;return b}).then(b=>{b=new Book(b);$('[data-element="book-information"]').html(`<div class="row mx-0 mb-8 p-8 roundBox" data-googleid="${b.googleId}">
        ...
        </div>`);return b}).then(async b=>{$('[data-element="help"]').html("Loading offers...");return await fetch(`/api/v1/market/detail?bookid=${h}`,{method:"GET",mode:"cors",headers:csrfprotect({"Content-Type":"application/json"})}).then(c=>{if(c.ok)return c.json();throw new NetworkError(c);}).then(c=>{if("success"!=c.status)throw new APIError(c);if(0==c.data.length)throw new APIError(c);return c.data}).then(c=>{let a=$('[data-element="offer_results"]');for(const e of c){var d=new Listing(e);const f=
new k(e.owner);d=$(`<div class="row mx-0 mb-8 p-16 roundBox listing" data-listingid="${d.listingid}" data-owneruserid="${f.userid}" data-invalid="${!!e.invalid}">
                    ...
                    <div class="row mt-0 mb-2">
                        ...
                        <div class="col s5">
                            ...
                            <div class="row my-0">
                                <div class="col s12 word-wrap font-size-16 right-align">${d.strings.created}</div>
                            </div>
                            <div class="row my-0 justify-content-end flex-wrap">
                                <div class="col">
                                    ...
                                    <div class="row my-0">
                                        <div class="col s12 word-wrap font-size-16 right-align">
                                            <span class="${d.strings.conditionClass} tooltipped" data-position="left" data-tooltip="${d.strings.conditionDescription}">
                                                ${d.strings.condition}
                                            </span>
                                        </div>
                                    </div>
                                ...
                                <div class="col">
                                    ...
                                    <div class="row my-0">
                                        <div class="col s12 word-wrap font-size-16 right-align">${d.strings.notes}</div>
                                    </div>
                                </div>
                                <div class="col">
                                    <div class="row my-0">
                                    ...
                                    <div class="row my-0">
                                        <div class="col s12 word-wrap font-size-16">${d.strings.price}</div>
                                    </div>
                                </div>
                            </div>
                            <div class="row my-0">
                            ...
                            <div class="row my-0">
                                <div class="col s12 word-wrap font-size-16 right-align">${d.strings.remarks}</div>
                            </div>
                        </div>
                    </div>
                    <div class="row my-0 justify-content-end">
                        <div class="col">
                            ...
                            <a class="btn px-8 roundBox btn-transparent ${e.invalid?"btn-transparent-disabled tooltipped":"btn-transparent-primary"}" data-button="create_offer" data-position="top" data-tooltip="${e.invalid}">
                                <i class="material-icons left">shopping_cart</i>
                                Make Offer
                            </a>
                        </div>
                    </div>
                </div>`);a.append(d)}$('[data-button="view_profile"]').click(e=>{e=$(e.target).closest("[data-owneruserid]").attr("data-owneruserid");window.location.href=`/profile/${e}`});$('[data-button="view_image"]').click(e=>{const f=$("#carousel").empty(),g=$(e.target).closest("[data-listingid]").attr("data-listingid");c.find(l=>l.listingid==g).images.forEach(l=>{f.append(`<a class="carousel-item justify-content-center"><img src="${l}"></a>`)});$("#imagemodal").modal("open")});$('[data-button="create_offer"]').click(e=>
{e=$(e.target).closest("[data-listingid]");"true"!=e.attr("data-invalid")&&(e=e.attr("data-listingid"),$('[data-button="create_offer_confirm"]').attr("data-listingid",e),$("#createmodal").modal("open"))});$('[data-button="create_offer_confirm"]').click(e=>{const f=$(e.target).attr("data-listingid");fetch("/api/v1/offer/create",{method:"POST",mode:"cors",headers:csrfprotect({"Content-Type":"application/json"}),body:JSON.stringify({listingid:f})}).then(g=>{if(!g.ok)throw new NetworkError;return g.json()}).then(g=>
{if("success"!=g.status)throw new APIError(g);return g.data}).then(()=>{$(e.target).removeClass("btn-transparent-primary").addClass("btn-transparent-disabled tooltipped");toast(description='Successfully created offer. Please go to the <a href="/dashboard">dashboard</a> for further steps.',headerPrefix="",code=1)}).catch(g=>{toastError(g)}).finally(()=>{$("#createmodal").modal("close")})});$('[data-element="help"]').empty()}).catch(c=>{throw c;})}).catch(b=>{b instanceof NoGoogleBooksResultsError?
