...
"None";break;case 1:this.strings.notes="Some";break;case 2:this.strings.notes="Comprehensive"}this.strings.price=`HKD ${this.price}`;this.strings.remarks=this.remarks||"---"}};class h extends Error{constructor(a){super(a)}}class k extends Error{constructor(a){super(a);this.message=`${a.status}: ${a.statusText}`}}class b extends Error{constructor(a){super(a);this.message=`${a.code}: ${a.message}`}}class c extends Error{constructor(){super();this.message="No results found."}}window.NetworkError=k;window.APIError=
b;window.NoGoogleBooksResultsError=c;window.ControlledError=h;window.toast=function(a){const f=a.description||"An unknown error occurred",d=`${a.headerPrefix} `||"";let e,g;switch(a.code||3){case 3:e=`${d}Error`;g="toastError";break;case 2:e=`${d}Warning`;g="toastWarning";break;case 1:e=`${d}Success`;g="toastSuccess";break;case 0:e=`${d}Information`,g="toastInformation"}M.toast({unsafeHTML:`<div>
...
{method:"GET",mode:"cors",headers:{"Content-Type":"application/json"}}).then(b=>{if(!b.ok)throw new NetworkError(b);return b.json()}).then(b=>{if("error"in b)throw new NoGoogleBooksResultsError;return b}).then(b=>{b=new Book(b);$('[data-element="book-information"]').html(`<div class="row mx-0 mb-8 p-8 roundBox" data-googleid="${b.googleId}">
        ...
        </div>`);return b}).then(async b=>{$('[data-element="help"]').html("Loading offers...");return await fetch(`/api/v1/market/detail?bookid=${h}`,{method:"GET",mode:"cors",headers:csrfprotect({"Content-Type":"application/json"})}).then(c=>{if(c.ok)return c.json();throw new NetworkError(c);}).then(c=>{if("success"!=c.status)throw new APIError(c);if(0==c.data.length)throw new APIError(c);return c.data}).then(c=>{let a=$('[data-element="offer_results"]');for(const d of c){var f=new Listing(d);const e=
new k(d.owner);f=$(`<div class="row mx-0 mb-8 p-16 roundBox listing" data-listingid="${f.listingid}" data-owneruserid="${e.userid}" data-invalid="${!!d.invalid}">
                ...
                </div>`);a.append(f)}$('[data-button="view_profile"]').click(d=>{d=$(d.target).closest("[data-owneruserid]").attr("data-owneruserid");window.location.href=`/profile/${d}`});$('[data-button="view_image"]').click(d=>{const e=$("#carousel").empty(),g=$(d.target).closest("[data-listingid]").attr("data-listingid");c.find(l=>l.listingid==g).images.forEach(l=>{e.append(`<a class="carousel-item justify-content-center"><img src="${l}"></a>`)});$("#imagemodal").modal("open")});$('[data-button="create_offer"]').click(d=>
{d=$(d.target).closest("[data-listingid]");"true"!=d.attr("data-invalid")&&(d=d.attr("data-listingid"),$('[data-button="create_offer_confirm"]').attr("data-listingid",d),$("#createmodal").modal("open"))});$('[data-button="create_offer_confirm"]').click(d=>{const e=$(d.target).attr("data-listingid");fetch("/api/v1/offer/create",{method:"POST",mode:"cors",headers:csrfprotect({"Content-Type":"application/json"}),body:JSON.stringify({listingid:e})}).then(g=>{if(!g.ok)throw new NetworkError;return g.json()}).then(g=>
{if("success"!=g.status)throw new APIError(g);return g.data}).then(()=>{$(`[data-listingid="${e}"] [data-button="create_offer"]`).removeClass("btn-transparent-primary").addClass("btn-transparent-disabled tooltipped");toast({description:'Successfully created offer. Please go to the <a href="/dashboard">dashboard</a> for further steps.',headerPrefix:"",code:1})}).catch(g=>{toastError(g)}).finally(()=>{$("#createmodal").modal("close")})});$('[data-element="help"]').empty()}).catch(c=>{throw c;})}).catch(b=>
{b instanceof NoGoogleBooksResultsError?$('[data-element="help"]').html("This book does not exist."):b instanceof APIError?$('[data-element="help"]').html("There are no offers avaliable for the moment, please try again later."):b instanceof NetworkError?$('[data-element="help"]').html("An error occured when retrieving data. Please check your connection or try again."):console.error(b)}).finally(()=>{$('[data-element="progress"]').addClass("hide");$(".tooltipped").tooltip()})});
